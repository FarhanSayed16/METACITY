# EvoCity Nexus — Frontend Implementation Plan

> **No code.** Pages, navigation, design system, motion, component structure, data mapping, and build order only.  
> Aligns with `EvoCity_Nexus_DSA_Build_Plan_v3.md` and `EvoCity_Nexus_Backend_Implementation_Plan.md`.

| Field | Value |
|---|---|
| Project | EvoCity Nexus (METACITY) |
| Document | Frontend Implementation Plan v1.0 |
...
- Wordmark **EvoCity Nexus** in chrome (left)  
...
| **Empty** | CTA “Create scenario” + link to flagship bypass preset |
| **Priority** | MVP |
...
| **Floating Layer Panel** | Toggles: roads, buildings, congestion, zones, transit, agents, utilities… |
| **View toggles** | 2D / 3D; legend; focus home extent |
...
- Edits: local scene draft; save explicit  
...
Onboarding tooltip first visit: controls cheat sheet.

### 14.4 Legends (always designed, not afterthought)
...
### Phase 0–1 (shell + first map)

1. Vite app, router, theme tokens, typography, Lucide  
2. AppShell, TopBar, StatusBar, basic Projects pages  
3. Welcome + Project Dashboard + New Project (template only)  
4. Workspace shell with empty canvas  
5. R3F ground + roads from scene JSON (static)  
6. API client + load template integration  

### Phase 2 (alive map)

1. WebSocket snapshots → congestion colours + agent dots  
2. SimControlStrip wired to play/pause/speed/seed display  
3. Layer panel  
4. 2D/3D camera toggle (basic)  
5. Link selection + inspector  

### Phase 3 (MVP complete UI)

1. Road edit tools + save  
2. Scenario library + builder  
3. Runs list + progress UX  
4. Comparison hub + result (table, charts, mechanisms, badge)  
5. Assumptions help  
6. Motion polish on routes/panels  
7. Empty/error/loading states everywhere  

### Phase 4

1. Evacuation workspace UI  
2. Hospital workspace UI  
3. Module entries in project nav  

### Phase 5

1. 3D walkthrough controls  
2. Day/night, split before/after  
3. LOD / presentation polish  

### Phase 6+

1. Extra layer toggles (utilities, emissions legends)  
2. Year-loop controls (simple)  
...
- [ ] Workspace: View + Edit(roads) + live/ snapshot congestion  
- [ ] 2D and basic 3D toggle  
- [ ] Sim strip: play/pause/step/reset/speed 1x–100x  
- [ ] Scenario builder validates and enqueues runs  
...
| Snapshot floods React | Isolate canvas store; minimise React state per frame |
| Dual MapLibre + R3F too early | Synthetic path only until Phase 7 |
| Inconsistent styling | Tokens + forbidden one-off colours in review |
...
| Presentation mode | Fullscreen hide chrome | Phase 5 |
...
*Companion to Build Plan v3 and Backend Implementation Plan. Keep this document code-free; update when routes or design tokens change.*
